...
| 阶段 3 | Week7-Week12 | 完成 Memory、UI、Voice、Repair、MCP、Skills | 已完成 |
| 阶段 4 | 收官阶段 | Demo、合规、复现性、文档交付 | 进行中 |
...
- 已完成并验收。
- 当前已新增 `POST /api/voice/run`，请求形态为 `multipart/form-data`。
- 当前服务端已补上：
  - `voice_input_received`
  - `voice_transcribed`
  - `voice_response_synthesized`
  - `voice_playback_ready`
- `voice_stt / voice_tts` 已从纯配置预留升级为代码层真实接缝：
  - 仍使用抽象接口
  - 后续可替换为本地、腾讯、Google 等实现
- 当前语音编排策略为：
  - STT / TTS 由 Voice Orchestration Service 负责
  - 中间文本回复继续复用现有 Agent Runtime
  - replay 继续沿用同一套事件与索引体系
- 当前真实联调已完成本地闭环：
  - 本地 `faster-whisper-server` 承担 `voice_stt`
  - 本地 `Kokoro` 承担 `voice_tts`
  - `wav` 音频上传可稳定返回 `mp3`
  - replay 可完整回放整条语音链路
...
- 已完成。
- 已引入 `FailureSource`：`planner / executor / tool / memory`
- 已新增 `IReviewer` 与 repair plan 模型，并保留现有 `ReflectionAgent / retry_scheduled` 兼容路径
- 当前事件链已补上：
  - `repair_plan_created`
  - `repair_step_started`
  - `repair_step_completed`
- 当前 replay 已投影 repair 摘要，并会继续反映 repair step 的实时状态
- Replay UI 已补充 repair 面板、repair filter 与状态摘要
- `planner / executor / tool / memory` 四类失败来源当前都已有真实测试样例覆盖
- Week11 验收 runbook 已补齐，可直接用于正式验收与 Week12 demo 取样
...
- 已开始实施。
- 当前第一阶段已开始收口 unified external tool governance：
  - external/MCP tool allowlist
  - `external_call_started / external_call_finished / external_call_blocked`
  - demo external tool `mcp.demo_echo`
- 当前第二阶段已开始接入最小 skill runtime：
  - `skill_selected`
  - `GET /api/skills`
  - demo skill `tech.mcp_demo`
  - replay detail skill 摘要投影
- 当前已新增 Week12 演示 runbook：
  - `Docs/03-modules/demo-runbook.md`
- 当前已新增 Week12 样例采集脚本：
  - `Scripts/week12/capture-demo-samples.sh`
  - `Scripts/week12/capture-demo-samples.ps1`
- 当前已新增 Week12 blocked-tool drill 脚本：
...
- Week9 已完成并通过真实环境验收：
  - 独立 Replay UI 工程已建立
  - Replay API 已可输出 `/api/replay/runs`、`/api/replay/runs/{runId}`、`/api/replay/runs/{runId}/events` 与 `/api/replay/suggestions`
  - replay 元数据已进入 PostgreSQL `replay_runs` 表，而不是继续停留在目录扫描方案
- Week10 已完成并通过本地真实环境验收：`voice_stt -> runtime -> voice_tts` 与 replay 事件链均已打通
- Week11 已正式验收通过：Reviewer / Repair Plan、repair 事件链、replay/detail/UI repair 展示、四类 failure source 测试样例均已收口

## 当前阻塞

- Week12 已正式验收通过：MCP / Skills / Demo 收口已经完成，当前主线进入收官阶段。
- 当前主要剩余的是收官类工作，以及 Daily Suggestion 的进一步个性化与自动调度启用策略。
