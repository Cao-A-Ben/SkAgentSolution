...
- `rerank / voice` 当前仅完成配置与契约预留，尚未接入真实调用链。
...
- Week8.5 第一阶段已通过真实环境验收：`planner / chat / daily` 的 `model_selected` 已与实际调用一致，`embedding` 也已接入统一路由的本地 hash 配置。

## 当前阻塞

- 当前主要剩余的是 Daily Suggestion 的进一步个性化、自动调度启用策略与失败补跑能力。
- Week8.5 还剩 `rerank / voice` 的真实接入，以及更细粒度的模型审计字段。

## 下一阶段计划

1. 继续增强 Daily Suggestion 的 persona/profile/facts 个性化，让建议更贴近真实项目推进。
2. 保持 `conversationId + date` 幂等不变，继续观察 `default` 与 `coach` 在次日 Daily Suggestion 上的差异。
3. 将 `rerank / voice` 继续纳入统一模型路由，并在真实调用链落地后补齐事件验收。
4. 再决定是否开启自动调度、失败补跑与更细的 provider-specific 审计字段。
