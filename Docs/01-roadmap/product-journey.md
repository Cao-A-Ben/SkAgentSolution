...
- DailySuggestionJob、SuggestionStore、手动触发接口和 SQL 尚未实现。
...
- Week8 尚未完成建议存储、手动触发接口和后台 Job 的实现。
- 建议回放需要补齐 JSONL 事件落盘与 suggestion record 持久化。

## 下一阶段计划

1. 提交 Week7 收口代码和文档。
2. 实现 Week8 的 SuggestionRecord、SuggestionStore、DailySuggestionService、DailySuggestionJob。
3. 增加手动触发接口和 SQL 脚本。
4. 完成 Week8 文档与事件验收。
