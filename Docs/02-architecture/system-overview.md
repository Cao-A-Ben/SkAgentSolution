...
- Week9 将新增独立前端 Replay UI；Week10 再接入 Voice Panel 与语音入口。
...
- Week9 正在聚焦独立前端 Replay UI；当前已补上 Replay API 与 `replay_runs` 元数据索引表，Week10 将聚焦 Voice 对话 MVP。
- Reviewer / Repair Plan、MCP、领域 Skills 作为后续阶段继续推进。

### 暂未落地

- Daily Suggestion 的内容质量进一步个性化与自动调度启用策略。
- `voice_stt / voice_tts` 的真实模型接入与更细的模型审计字段。
- 外部 MCP 适配器与权限体系。
