...
- Week9 已落地独立前端 Replay UI；Week10 正在接入 Voice API 与语音入口。
...
- Week9 Replay UI 已完成并进入已验收状态；Week10 正在推进 Voice 对话 MVP 第一阶段。
- Week10 当前已补上 `POST /api/voice/run` 与默认 `openai-compatible` STT / TTS 接缝，同时保持 provider-neutral 抽象。
- Week10 当前也已补上独立 `VoiceGateway` 配置入口，并把外部语音 provider 故障收敛为标准 `502 problem+json`。
- Week10 当前进一步切分为：
  - `voice_stt` 可继续保留远程或后续接本地 Whisper
  - `voice_tts` 默认走本地 Kokoro
- Reviewer / Repair Plan、MCP、领域 Skills 作为后续阶段继续推进。
