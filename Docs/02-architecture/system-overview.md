...
- API 与 SSE 面向对话、流式输出和回放入口。
- Week9 已落地独立前端 Replay UI；Week10 已落地 Voice API 与最小语音入口。
...
- Infrastructure 提供 Postgres / pgvector、Embedding、Event Store、Suggestion Store、Replay Run Store 等实现。
- Observability 记录事件、指标、脱敏结果和回放素材。

补充说明：

- Week9 起，replay 采用 `PostgreSQL + JSONL` 混合架构。
- `replay_runs` 表负责 run 元数据索引、列表读取与 `runId -> eventLogPath` 定位。
- JSONL 继续保存原始事件 envelope，作为 timeline / prompt / steps / memory 的事实源。
...
- Week8.5 多模型适配层：`planner / chat / daily / embedding / rerank` 已完成真实验收；progress recall 也已通过真实环境收口。
- Week9 Replay UI 已完成并进入已验收状态；Week10 Voice 对话 MVP 也已完成本地真实验收。
- Week10 当前已补上 `POST /api/voice/run`，并保持 provider-neutral 抽象。
- Week10 当前默认闭环为：
  - `voice_stt` -> 本地 `faster-whisper-server`
  - 中间回复 -> 复用现有 `AgentRuntimeService`
  - `voice_tts` -> 本地 `Kokoro`
- Week10 当前 replay 已能稳定记录：
  - `voice_input_received`
  - `voice_transcribed`
  - `voice_response_synthesized`
  - `voice_playback_ready`
- Week11 已完成实现接入 Reviewer / Repair Plan，待正式验收：
  - 失败来源统一收敛为 `planner / executor / tool / memory`
  - replay 已记录 `repair_plan_created / repair_step_*`
  - Replay UI 已可展示 repair 来源、原因、步骤与实时状态摘要
- MCP、领域 Skills 作为后续阶段继续推进。

### 暂未落地

- Daily Suggestion 的内容质量进一步个性化与自动调度启用策略。
- 语音前端播放体验、流式音频下发与更细的语音 provider 审计字段。
- 外部 MCP 适配器与权限体系。
