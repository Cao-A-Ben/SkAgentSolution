...
- Status: Week8.5 Accepted, Week10 Accepted
- Owner: Ben + Codex
...
- `voice_stt / voice_tts` 已进入真实运行链路，并已完成 Week10 本地真实环境验收。
...
| `voice_stt` | `Systran/faster-whisper-small` | 已接入并验收，默认对接本地 faster-whisper-server |
| `voice_tts` | `tts-1` | 已接入并验收，默认对接本地 Kokoro |
...
- `model_selected purpose=voice_stt -> Systran/faster-whisper-small`
- `voice_transcribed`
- `model_selected purpose=voice_tts -> tts-1`
...
- `voice_stt / voice_tts` 现已进入真实代码链路，并已完成最小闭环验收；后续仍可继续扩充 provider-specific 审计字段。
...
2. 为 Voice replay 增加更细粒度的 provider-specific 诊断字段。
