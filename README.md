...
- [Final Delivery Summary](Docs/03-modules/final-delivery-summary.md)
...
- 当前语音链路为：
  ...
  - 复用现有 Agent Runtime
  - `voice_tts`
- 当前默认实现为 provider-neutral 抽象 + 本地默认 provider
...
- 当前默认 `voice_tts` 已切到本地 `Kokoro`（`http://127.0.0.1:8880`）
- 当前已完成真实闭环验收：
  - `wav` 音频上传
  - STT 转写
  - Runtime 文本回复
...
- 已引入 `FailureSource`：
  ...
  - `tool`
  - `memory`
- 已新增 `IReviewer` 与规则型 reviewer 实现，保留现有 `ReflectionAgent / retry_scheduled` 兼容路径。
- 当前 repair 事件链已接入：
  - `repair_plan_created`
  - `repair_step_started`
