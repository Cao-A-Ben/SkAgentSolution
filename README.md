...
产品级 Agent Runtime 项目，Week7 已完成并通过真实环境验收，当前进入 Week8：Daily Suggestion Job foundation 已完成编码与测试，正在做真实环境验收。
...
- 当前优先级是完成 Week8：Daily Suggestion Job 的真实环境验收、SQL 执行与接口联调。
...
3. 先阅读文档主线，确认当前阶段和结构。
4. 如需继续推进，请先看 Week7 验收结果，再查看 Week8 的 Daily Suggestion foundation 与验收步骤。
...
4. 查看 [Week7 Acceptance Runbook](Docs/03-modules/week7-acceptance-runbook.md) 的已完成结论，再读 [Daily Suggestions](Docs/03-modules/daily-suggestions.md) 进入 Week8。
