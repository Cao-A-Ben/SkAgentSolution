...
namespace SKAgent.Runtime
{
    ...
    /// - 最终输出和运行状态
    /// </summary>
    public sealed class AgentRunContext : IRunContext
    {
        #region fileds

        #region  IRUNContext 必须字段。
        /// <summary>
        ...
        #region ========== Runtime 扩展：Root/事件/准备快照等 ==========
        /// <summary>
        ...
        /// 可被 Host 注入 Jsonl/SSE/Composite）。
        /// </summary>
        ...
        /// 当前事件序号快照（只读）。
        /// </summary>
        public long EventSeq => Volatile.Read(ref _eventSeq);
        ...
        public RuntimePreparation Prep { get; } = new();
        ...
        public ValueTask EmitAsync(string type, object payload, CancellationToken ct = default)
        {
            ...
            return EventSink.WriteAsync(ev, ct);
        }
        ...
        public void MarkFailed(string? finalOutput = null)
        {
            FinalOutput = finalOutput;
            Status = AgentRunStatus.Failed;
        }

        // PlanExecutor 内部使用：追加 step
        internal void AddStepExecution(PlanStepExecution exec) => Steps.Add(exec);


        // PlanExecutor 内部使用：更新 retry 计数
        internal void IncrementRetry(int stepOrder)
        {
            StepRetryCounts.TryGetValue(stepOrder, out var c);
            StepRetryCounts[stepOrder] = c + 1;
        }
    }
}
